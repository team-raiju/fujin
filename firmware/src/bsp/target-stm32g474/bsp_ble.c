...
{
    MX_USART1_UART_Init();
    BSP_UART_Register_Callback(UART_NUM_1, uart_callback);
    BSP_UART_Register_Error_Callback(UART_NUM_1, uart_error_callback);
...
void bsp_ble_start(void)
{
    if (!ble_running) {
        ble_running = true;
        HAL_UART_Receive_DMA(&huart1, ble_dma_data, sizeof(ble_dma_data));
    }
}
