...
def main():
    ...
    angular_acceleration_rad_s2 = 104.71975
    angular_desacceleration_rad_s2 = 104.71975
    maximum_angular_speed_rad_s = 10.47197
    ...
    print("Ideal angular speed (rad/s): {:.2f} (={:.2f} deg/s)".format(ideal_angular_speed_rad_s, ideal_angular_speed_deg_s))
    print("")
    ...
    print("Max angular speed (rad/s): ", maximum_angular_speed_rad_s, " (=", np.rad2deg(maximum_angular_speed_rad_s), "deg/s)")
    print("Acceleration (rad/s^2): ", angular_acceleration_rad_s2, " (=", np.rad2deg(angular_acceleration_rad_s2), "deg/s^2)")
    print("Deceleration (rad/s^2): ", angular_desacceleration_rad_s2, " (=", np.rad2deg(angular_desacceleration_rad_s2), "deg/s^2)")
    ...
    for t in range(1, int(total_ideal_time_ms) + 1):
        ...
        ideal_ang_speeds.append(angular_speed_rad_s)
    ...
    for t in range(1, round(T_ms + 1)):
        ...
        if (t == round(t1_ms)):
            print(f"Reached max angular speed at t = {t} ms and angle = {np.rad2deg(real_positions[-1].theta):.3f} deg -> {real_positions[-1].theta:.3f} rad")
        elif (t == round(t1_ms + t2_ms)):
            print(f"Started deceleration at t = {t} ms and angle = {np.rad2deg(real_positions[-1].theta):.3f} deg -> {real_positions[-1].theta:.3f} rad")
